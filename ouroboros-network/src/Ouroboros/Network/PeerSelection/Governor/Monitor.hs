...
connections PeerSelectionActions{
    ...
    Guarded Nothing $ do
      ...
      return $ \now ->
        let (aFuzz, rng')      = randomR (-5, 5 :: Double) fuzzRng
            (rFuzz, rng'')     = randomR (-1, 3 :: Double) rng'
            ...
            establishedPeers'  = EstablishedPeers.setActivateTime
                                  ...
                                  ((realToFrac aFuzz + activateDelay) `addTime` now)
            ...
            knownPeers'        = KnownPeers.setConnectTime
                                   (Map.keysSet demotedToCold)
                                   ((realToFrac rFuzz + reconnectDelay) `addTime` now)
                               . Set.foldr'
